...
const seedDB = async () => {
    try {
        ...
        const hashedAdminPassword = await (0, auth_1.hashPassword)('admin123');
        const hashedWorkerPassword = await (0, auth_1.hashPassword)('worker123');
        const hashedVolunteerPassword = await (0, auth_1.hashPassword)('volunteer123');
        const userData = [
            {
                name: 'NGO Admin',
        ...
        for (const u of userData) {
            await User_1.default.findOneAndUpdate({ email: u.email }, u, { upsert: true, new: true });
        }
        const adminUser = await User_1.default.findOne({ email: 'admin@nexus.com' });
        const workerUser = await User_1.default.findOne({ email: 'worker@nexus.com' });
        const volunteerUser = await User_1.default.findOne({ email: 'volunteer@nexus.com' });
        ...
        const surveys = await Survey_1.default.insertMany([
            {
        ...
        await Task_1.default.insertMany(dummyTasks);
