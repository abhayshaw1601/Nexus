...
export const saveDraft = async (req: Request, res: Response) => {
  try {
    ...
    const { surveyId, title, description, category, urgency, affectedPeople, location, imageUrls, extractedEntries } = req.body;
    ...
    if (surveyId && mongoose.isValidObjectId(surveyId)) {
      survey = await Survey.findOneAndUpdate(
        ...
        { title, description, category, urgency, affectedPeople, location, imageUrls, extractedEntries, ngoId },
        { new: true }
      );
    }
