...
export default function VerificationQueuePage() {
  ...
  const fetchPendingCompletionReports = async () => {
    try {
      const res = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/completion-reports/pending`, {
        headers: { Authorization: `Bearer ${(session?.user as any).accessToken}` }
      });
      setCompletionReports(res.data);
    } catch { console.error("Failed to fetch completion reports"); }
  };

  const handleVerifySurvey = async (surveyId: string, action: 'VERIFIED' | 'REJECTED') => {
    try {
  ...
  const handleVerifyCompletion = async (reportId: string, action: 'VERIFIED' | 'REJECTED') => {
    try {
      await axios.post(`${process.env.NEXT_PUBLIC_API_URL}/completion-reports/${reportId}/verify`, { action }, {
        headers: { Authorization: `Bearer ${(session?.user as any).accessToken}` }
      });
      setCompletionReports(completionReports.filter(r => r._id !== reportId));
    } catch { alert(`Failed to ${action.toLowerCase()} completion report`); }
  };
  ...
  const tabBtn = (tab: 'surveys' | 'completions', label: string, icon: any, count: number) => (
    <button
    ...
    >
      ...
      {label}
      <span style={{ 
        backgroundColor: activeTab === tab ? '#FFFFFF' : PUR, 
        color: activeTab === tab ? PUR : '#FFFFFF',
        padding: '2px 8px',
        borderRadius: '4px',
        fontSize: '0.65rem',
        fontWeight: 900
  ...
  const currentItems = activeTab === 'surveys' ? surveys : completionReports;
      ...
      <main className="neo-main">
        ...
        <div style={{ marginBottom: '2rem', paddingBottom: '1.5rem', borderBottom: `2.5px solid ${BLACK}` }}>
          ...
          <p className="page-subtitle">
            Review and approve field reports and task completions.
          </p>
        </div>

        {/* Tab Navigation */}
        <div style={{ display: 'flex', gap: '1rem', marginBottom: '2rem', flexWrap: 'wrap' }}>
          {tabBtn('surveys', 'Field Reports', <FileText style={{ width: 16, height: 16 }} />, surveys.length)}
          {tabBtn('completions', 'Task Completions', <ClipboardCheck style={{ width: 16, height: 16 }} />, completionReports.length)}
        </div>
          ...
          {currentItems.length === 0 ? (
            <div style={{ ...CARD, padding: '5rem', textAlign: 'center', width: '95%', margin: '0 auto' }}>
              ...
              <p style={{ fontFamily: "'Space Mono',monospace", fontSize: '0.7rem', color: 'var(--muted-fg)' }}>
                No pending {activeTab === 'surveys' ? 'field reports' : 'completion reports'} detected.
              </p>
            </div>
          ) : currentItems.map((item) => {
            const urgency = item.urgency || item.urgencyScore || 1;
            const isCompletion = activeTab === 'completions';
            
            return (
              <div key={item._id} className="neo-card card-row" style={{ 
                padding: '24px', 
                display: 'flex', 
                gap: '20px', 
                marginBottom: '16px', 
                ...
                <div style={{ flex: 0.7, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
                  ...
                  <div className="neo-scroll-content" style={{ flexGrow: 1, paddingRight: '12px', marginBottom: 16, overflowY: 'auto' }}>
                    ...
                    <h3 style={{ 
                      fontFamily: "'Plus Jakarta Sans',sans-serif", 
                      fontWeight: 900, 
                      fontSize: isCompletion ? '1.2rem' : '1.4rem', 
                      textTransform: 'uppercase', 
                      color: FG, 
                      margin: 0, 
                      lineHeight: 1.2,
                    ...
                    {/* Affected People for completion reports */}
                    {isCompletion && item.affectedPeople && (
                      <div style={{ marginTop: 12, padding: '8px 12px', backgroundColor: 'var(--bg)', border: `1.5px solid ${BLACK}` }}>
                        <span style={{ fontFamily: "'Space Mono',monospace", fontSize: '0.65rem', color: 'var(--muted-fg)' }}>
                          AFFECTED: <strong style={{ color: FG, fontWeight: 900 }}>{item.affectedPeople} people</strong>
                        </span>
                      </div>
                    )}

                    {/* Proof images for completion reports */}
                    {isCompletion && item.proofImageUrls?.length > 0 && (
                      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginTop: 12 }}>
                        {item.proofImageUrls.map((url: string, i: number) => (
                          <a key={i} href={url} target="_blank" rel="noreferrer">
                            <img
                              src={url}
                              alt={`proof-${i}`}
                              style={{ width: 80, height: 80, objectFit: 'cover', border: `2px solid ${BLACK}`, cursor: 'pointer' }}
                            />
                          </a>
                        ))}
                      </div>
                    ...
                    {/* Data stack if it exists (for surveys) */}
                    {!isCompletion && item.dataStack && Object.keys(item.dataStack).length > 0 && (
                      <div style={{ backgroundColor: 'var(--bg)', border: `1.5px solid ${BLACK}`, padding: '0.75rem', marginTop: 12 }}>
                        <pre style={{ fontFamily: "'Space Mono',monospace", fontSize: '0.65rem', color: FG, overflowX: 'auto', margin: 0 }}>
                          {JSON.stringify(item.dataStack, null, 2)}
                        </pre>
                      </div>
                    )}
                  </div>

                  {/* Row 3: Metadata (Worker/Volunteer + GPS) - ANCHORED AT BOTTOM OF LEFT COL */}
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 6, flexShrink: 0, paddingTop: 12, borderTop: `1px solid rgba(0,0,0,0.05)` }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, color: 'var(--muted-fg)' }}>
                      ...
                      <span style={{ fontFamily: "'Space Mono',monospace", fontSize: '0.6rem', textTransform: 'uppercase' }}>
                        {isCompletion ? 'Volunteer' : 'Worker'}: <strong style={{ color: FG, fontWeight: 900 }}>
                          {isCompletion ? (item.volunteerId?.name || 'Unknown') : (item.fieldWorkerId?.name || 'Unknown')}
                        </strong>
                      </span>
                    </div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, color: 'var(--muted-fg)' }}>
                      ...
                      <span style={{ fontFamily: "'Space Mono',monospace", fontSize: '0.6rem', textTransform: 'uppercase' }}>
                        GPS: <strong style={{ color: FG, fontWeight: 900 }}>
                          {item.location?.coordinates?.length >= 2 ? `${Number(item.location.coordinates[1]).toFixed(4)}, ${Number(item.location.coordinates[0]).toFixed(4)}` : 'N/A'}
                        </strong>
                      </span>
                    </div>
                  </div>
                ...
                <div style={{ 
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 12, width: '100%' }}>
                    <button
                      onClick={() => isCompletion ? handleVerifyCompletion(item._id, 'VERIFIED') : handleVerifySurvey(item._id, 'VERIFIED')}
                      style={{ 
                        fontFamily: "'Plus Jakarta Sans',sans-serif", 
                        fontWeight: 900, 
                        fontSize: '0.75rem', 
                    ...
                    <button
                      onClick={() => isCompletion ? handleVerifyCompletion(item._id, 'REJECTED') : handleVerifySurvey(item._id, 'REJECTED')}
                      style={{ 
                        fontFamily: "'Plus Jakarta Sans',sans-serif", 
                        fontWeight: 900, 
                        fontSize: '0.75rem', 
